...
const StageClip = ({ clip, currentTime, isPlaying, isSelected, onSelect, onContextMenu, onTransform, setEditingTextId }: { 
    ...
    useEffect(() => {
        if ((clip.type === 'video' || clip.type === 'audio') && videoRef.current) {
            ...
            // 3. Sync Volume
            media.volume = clip.properties.volume || 1.0;
        }
    }, [currentTime, isPlaying, clip.start, clip.duration, clip.type, clip.properties.volume]);
...
export default function EditorPage({ params }: { params: { id: string } }) {
  ...
  const checkTaskStatus = async (taskId: string) => {
      const interval = setInterval(async () => {
          try {
              // FIXED: Uses the correct dynamic endpoint helper from api.ts
              const res = await api.get(endpoints.tasks.get(taskId));
              ...
              } else if (res.data.status?.toLowerCase().includes('error')) {
                  clearInterval(interval);
                  setIsExporting(false);
                  alert(`Export Failed: ${res.data.status}`);
              }
